...
class Agentes_model extends CI_Model {
    ...
    public function numRegs($vcBuscar, $area=1, $cargo=0) {
        $sql = 'SELECT count(idAgente) AS inCant 
        FROM hits_personas p 
        INNER JOIN sigep_agentes a ON p.idPersona = a.idPersona
        WHERE lower(nombrePersona) LIKE ?;';
    ...
    /*public function obtener($vcBuscar = '', $area = 1, $cargo = 0, $limit = 0, $offset = 9999999) {
        ...
        $sql = 'SELECT *
            ...
            ORDER BY nombreCompletoPersona ASC  
            limit ? offset ? ;';
        return $this->db->query($sql, array('%' . strtolower((string) $vcBuscar) . '%', (double) $offset, (double) $limit))->result_array();
    }*/
    ...
    public function definitivo($id) {
        $sql = 'DELETE FROM sigep_agentes WHERE idAgente = ?;';
    ...
    public function dropdownAgentesAntecedentes() {
        $sql = 'SELECT * FROM sigep_agentes_antecedentes_motivos';
        $query = $this->db->query($sql)->result();
        $funciones[0] = 'Seleccione un item...';
        foreach($query as $row) {
            $funciones[$row->idMotivoAntecedente] = $row->nombreMotivoAntecedente; 
        }
        return $funciones;
    }
    ...
    public function cumpleanosProximosAgentes($vcBuscar = '', $limit = 0, $offset = 9999999) {
        $sql = 'SELECT *
            FROM sigep_view_agentes
            WHERE MONTH(nacimientoPersona) = MONTH(CURDATE()) AND DAY(nacimientoPersona) > DAY(CURDATE())
            ORDER BY RIGHT(nacimientoPersona,5) ASC
            LIMIT ? offset ? ;';
        return $this->db->query($sql, array((double) $offset, (double) $limit))->result_array();
    }
    public function cumpleanosHoyAgentes($vcBuscar = '', $limit = 0, $offset = 9999999) {
        $sql = 'SELECT *
            FROM sigep_view_agentes
            WHERE RIGHT(nacimientoPersona,5) = RIGHT(CURDATE(),5)
            ORDER BY apellidoPersona
            LIMIT ? offset ? ;';
        return $this->db->query($sql, array((double) $offset, (double) $limit))->result_array();
    }
