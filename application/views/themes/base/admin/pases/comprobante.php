<?php
	...
	$ci->pdf->Cell(84, 7, 'N° de Actuación SGA (SIGEP)', 1, 0, 'L', TRUE);
	$ci->pdf->Cell(84, 7, 'N° de Actuación SICE', 1, 0, 'L', TRUE);
	$ci->pdf->Cell(22, 7, 'Folios Total', 1, 1, 'L', TRUE);
	$ci->pdf->Cell(84, 7, $actuacion['codigoActuacion'], 1, 0, 'L', FALSE);
	$ci->pdf->Cell(84, 7, $actuacion['referenciaActuacion'], 1, 0, 'L', FALSE);
	$ci->pdf->Cell(22, 7, $actuacion['foliosActuacion']['foliosActuacion'], 1, 1, 'L', FALSE);

	$ci->pdf->Cell(84, 7, 'Tipo de documento o actuación', 1, 0, 'L', TRUE);
	$ci->pdf->Cell(84, 7, 'Motivo de Pase', 1, 0, 'L', TRUE);
	$ci->pdf->Cell(22, 7, 'Folios Agreg.', 1, 1, 'L', TRUE);
	$ci->pdf->Cell(84, 7, $actuacion['nombreActuacionTipo'], 1, 0, 'L', FALSE);
	$ci->pdf->Cell(84, 7, $pase['nombreActuacionPaseTipo'], 1, 0, 'L', FALSE);
	$ci->pdf->Cell(22, 7, $pase['foliosActuacionPase'], 1, 1, 'L', FALSE);
	...
	$ci->pdf->Cell(190, 7, 'Carátula', 1, 1, 'L', TRUE);
	$ci->pdf->Cell(190, 7, html_entity_decode(word_limiter($actuacion['caratulaActuacion'], 25)), 1, 1, 'L', FALSE);
	
	$ci->pdf->Cell(190, 7, 'Observaciones', 1, 1, 'L', TRUE);
	$ci->pdf->Cell(190, 7, html_entity_decode(word_limiter($pase['observacionActuacionPase'], 25)), 1, 1, 'L', FALSE);
	//$ci->pdf->Multicell(190, 7, $pase['observacionActuacionPase'], 1, 'L', false, 1, '', '', true, 0, true);
	...
	$ci->pdf->Cell(95, 5, 'Impreso por : '.$this->autenticacion->apellidoPersona().', '.$this->autenticacion->nombrePersona(), '', 0
		, 'L', FALSE);
	$ci->pdf->Cell(95, 5, 'Fecha de Impresión : '.date('d/m/y H:i:s'), '', 1, 'R', FALSE);
	$ci->pdf->Ln(7);
	$ci->pdf->Line(0,140,220,140);
