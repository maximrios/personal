...
class Ext_Pagination extends CI_Pagination {
    ...
    public $next_link          = '&raquo;';
    public $prev_link          = '&laquo;';
    public $empty_first_element = '<li class="disabled"><a href="#">&laquo;</a></li>';
    public $empty_last_element = '<li class="disabled"><a href="#">&raquo;</a></li>';
    /**
    * Constructor
    *
    * @access public
...
$vcXHtml = '<div class="informacion-paginador pull-right"><span class="item-contenedor">Cantidad total de registros '. $this->total_rows.'</span>'.
               '<span class="item-separador">|</span>'.
               '<span class="item-contenedor">Mostrando página '.$this->cur_page.' de '.$num_pages.'</span><span class="item-separador">&nbsp;</span></div>';
return $vcXHtml;
}
...
$this->next_tag_open = $this->num_tag_open;
$this->next_tag_close = $this->num_tag_close;
$this->prev_tag_open = $this->num_tag_open;
$this->prev_tag_close = $this->num_tag_close;
$this->cur_tag_open = '<li class="active">';
...
if ($this->prev_link !== FALSE AND $page_selected > 1) {
    ...
    if ($i == 0 && $this->first_url != '') {
        $output .= $this->prev_tag_open.'<a href="#" ic-post-to="'.$this->first_url.'" ic-target="#main_content">'.$this->prev_link.'</a>'.$this->prev_tag_close;
    }
    else {
        $i = ($i == 0) ? '' : $this->prefix.$i.$this->suffix;
        $output .= $this->prev_tag_open.'<a href="#" ic-post-to="'.$this->base_url.$i.'" ic-target="#main_content">'.$this->prev_link.'</a>'.$this->prev_tag_close;
    }
} 
else {
    ...
    for ($loop = $start ; $loop <= $end; $loop++) {
        ...
        if ($i >= 0) {
            if ($page_selected == $loop) {
                $output .= $this->cur_tag_open.'<a href="#" ic-post-to="'.$this->base_url.'" ic-target="#main_content">'.$loop.'</a>'.$this->cur_tag_close; // Current page
            }
            else {
                $n = ($i == 0) ? '' : $i;
                if ($n == '' && $this->first_url != '') {
                    $output .= $this->num_tag_open.'<a href="#" ic-post-to="'.$this->first_url.'" ic-target="#main_content" href="#">'.$loop.'</a>'.$this->num_tag_close;
                }
                else {
                    $n = ($n == '') ? '' : $this->prefix.$n.$this->suffix;
                    $output .= $this->num_tag_open.'<a href="#" ic-post-to="'.$this->base_url.$n.'" ic-target="#main_content">'.$loop.'</a>'.$this->num_tag_close;
                }
            }
        }
    }
...
if ($this->next_link !== FALSE AND $page_selected < $num_pages) {
    $output .= $this->next_tag_open.'<a '.$this->anchor_class.'href="#" ic-post-to="'.$this->base_url.$this->prefix.($page_selected + 1).$this->suffix.'" ic-target="#main_content">'.$this->next_link.'</a>'.$this->next_tag_close;
}
