...
class Pdf extends TCPDF {
	...
	function __construct($config) {
		$ci = &get_instance();
		//print_r($ci->session->userdata('nombrePersona'));
		...
		$this->impreso = $ci->session->userdata('nombrePersona');
    ...
    public function Footer() {
        ...
        // Page number
        $this->Cell(50, 10, 'Usuario : '.$this->impreso, 0, false, 'L', 0, '', 0, false, 'T', 'M');
        if($this->page_orientation == 'P') {
        	$this->Cell(90, 10, 'Página '.$this->getAliasNumPage().'/'.$this->getAliasNbPages(), 0, false, 'C', 0, '', 0, false, 'T', 'M');
        }
        else {
        	$this->Cell(170, 10, 'Páginaas '.$this->getAliasNumPage().'/'.$this->getAliasNbPages(), 0, false, 'C', 0, '', 0, false, 'T', 'M');	
        }
        $this->Cell(60, 10, 'Fecha y Hora de impresión : '.date('d-m-Y H:i:s'), 0, false, 'R', 0, '', 0, false, 'T', 'M');
    }
