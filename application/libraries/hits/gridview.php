...
class Gridview {
	 ...
	 * 
     */	
    var $sResponseType = 'GET';/* (GET|POST) */
...
.$sHeadContainerCl;
        ...
        if($this->_aTemplates['listElement']=='table') {
            //if(count($this->generales) > 0) {
                $columnas = count($this->aCols) - 3;    
            //}
            //else {
            //    $columnas = count($this->aCols);
            //}
            
        	/*$sXHtml = '<thead><tr><th colspan="'.$columnas.'" class="hilite">'.$this->titulo.'</th><th colspan="3"><form id="form-search-gridview" name="form-search-gridview" ic-post-to="'.$this->sResponseUrl.'" ic-target="#main_content"><div class="input-group search-gridview pull-right">
            <input type="text" id="buscarGridview" name="buscarGridview" class="form-control" placeholder="Buscar..."><span class="input-group-btn"><button class="btn btn-default" type="submit"><span class="glyphicon glyphicon-search"></span></button></span></div></form></th></tr><tr>'.$sXHtml.'</tr></thead>';*/
...
sTpl;
     ...
     * @return void
     */
	public function addControl($name, $faceTpl, $title = '', $function = '')
	{
		$options = array();
		if(is_array($faceTpl))
        { $options = $faceTpl; } 
    ...
    */	
    ...
    {
      	$this->_CI->load->library('hits/messages');      
    	$baseTemplate = array(
            'listElement' => 'table'
            , 'rowElement' => 'tr'
            , 'cellElement' => 'td'
    ...
    */	
    public function setPaginator($config=array()) {
		...
		/*
		 * Detectar del uri_segment el parametro del numero de pagina
		 */
		$parmsUri = $this->_CI->uri->uri_to_assoc(4);
        if(isset($parmsUri[$this->sPageParmName])) {
			$this->addParm($this->sPageParmName, $parmsUri[$this->sPageParmName]);
            $config['page'] = $parmsUri[$this->sPageParmName];
		}
    ...
    */	
    ...
    public function doXHtml($aRegs) {
    	...
    	foreach ($aRegs as $Reg)
            ...
            $sContainerOp = (!empty($this->_aTemplates['listElement']))?'<'.$this->_aTemplates['listElement'].' %s class="table table-striped table-hover">':''; 
