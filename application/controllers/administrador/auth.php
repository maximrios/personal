<?php
class Auth extends Ext_Controller {
	...
	function __construct() {
		...
		$this->load->model('admin/usuarios_model','_oModel');
	...
	protected function _init() {
		parent::_init();
		$this->load->library('hits/autenticacion');
	...
	public function autenticar() {
		...
		$vcResponse = '';
		if(!$this->autenticacion->logged(false)) {
			...
			if ($val->run() && $this->autenticacion->login($this->input->post('nombreUsuario'), $this->input->post('passwordUsuario'))) {
			    antibotBorrarLlaves();
                //$vcResponse = $this->autenticacion->doRedirect($this->autenticacion->opciones['login_success_uri']);
                $vcResponse = $this->autenticacion->doRedirect('admin');
            }
			else {
			...
			$vcResponse = $this->autenticacion->doRedirect($this->lib_autenticacion->opciones['login_uri']);
		}
		echo $vcResponse;
	}
	public function login() {
		//$dataView = array('txtLogin'=>$dni?$dni:'', 'vcMsjSrv' => $this->_getErrors());
        $this->autenticacion->unsetSession();
		...
		//$data['main_content'] = $this->load->view('admin/login', array(), true);
        //$data['vcMenu']='';
	...
	public function mesa() {
		$this->load->model('sigep/mesas_model' , 'mesas');
		$aData['mesas'] = $this->mesas->dropdownMesas($this->autenticacion->idAgente());
		$this->load->view('admin/mesa', $aData, FALSE);
	}
	public function ubicacion() {
		//antibotCompararLlave($this->input->post('vcForm'));
		$vcResponse = '';
		if($this->input->post('idMesaUsuario') > 0) {
			if($this->autenticacion->validar($this->input->post('idMesaUsuario'))) {
				//$vcResponse = $this->lib_autenticacion->doRedirect($this->lib_autenticacion->opciones['login_success_uri']);
				$vcResponse = $this->autenticacion->doRedirect('inicio');
			}
		}
		else {
			$this->mesa();
